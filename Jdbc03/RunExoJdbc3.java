...
public class RunExoJdbc3 {
	...
	private static void executeRequete(String maRequeteSelect, String monIp, String monUrl) {
		...
		try {
			...
			if((nombreColonne > 0) && (statement != null) && (resultat != null) && (metaData != null)) {
				...
				ToolsForFormatting.AlignLeft(6, "| n°");
				for (int i=0; i<nombreColonne; i++) {

					ToolsForFormatting.AlignLeft(tableauMax[i] + 4, "| " + tableauNomColonne[i]);
				...
				for(int j=0; j<listeTableauValeur.size(); j++) {
					ToolsForFormatting.AlignLeft(6, "| " + (j+1));
					for (int i=0; i<nombreColonne; i++) {
						ToolsForFormatting.AlignLeft(tableauMax[i] + 4, "| " + listeTableauValeur.get(j)[i]);
					}
					ToolsForFormatting.AlignLeft(1, "|");
					System.out.println();
				}
	...
	private static void afficheLigneTrait(int[] monTableauMax, int monNombreColonne) {
		ToolsForFormatting.AlignLeft(6, "+-----");

		for(int i=0; i<monNombreColonne; i++) {
			ToolsForFormatting.AlignLeft(monTableauMax[i] + 4, "+" + renvoieString('-',monTableauMax[i] + 3));
		}
		ToolsForFormatting.AlignLeft(1, "+");
		System.out.println();
